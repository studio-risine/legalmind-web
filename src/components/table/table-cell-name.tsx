...
export const TableCellPrimary = memo(({ link, title }: TableCellPrimaryProps) => {
	...
	return (
		<div className="group relative">
			...
			<div className="-top-1 invisible absolute right-0 z-10 rounded duration-100 ease-in-out group-hover:visible">
				<Button onClick={() => handleRowClick(link)} size="sm" variant="secondary">
...
function TableCellLabel({ children }: { children: ReactNode }) {
	return (
		<div className="max-w-[320px] truncate">
			<span className="truncate font-semibold text-base text-foreground">{children}</span>
		</div>
	)
}
