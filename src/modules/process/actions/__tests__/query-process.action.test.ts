...
const { userAuthAction } = await import('@modules/auth/actions/user-auth.action')
const { getProcessRepository } = await import('@modules/process/factories')
const { queryProcessByIdWithoutCacheAction: queryProcessAction } = await import(
	'../queries/query-process-by-id.action'
)
