...
describe("Get Space By ID Action", () => {
	...
	describe("Negative Scenarios", () => {
		...
		it("should return null when database query fails", async () => {
			...
			vi.mocked(getCurrentAccountId).mockResolvedValue("user-account-id")
			vi.mocked(db.query.spaces.findFirst).mockRejectedValue(new Error("Database connection failed"))
	...
	describe("Edge Cases", () => {
		it("should handle malformed space data gracefully", async () => {
			...
			vi.mocked(getCurrentAccountId).mockResolvedValue("user-account-id")
			vi.mocked(db.query.spaces.findFirst).mockResolvedValue(malformedSpace as any)
